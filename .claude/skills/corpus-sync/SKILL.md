...
description: >
  ...
  current corpus, regenerates phase-corpus.jsonl, and flags exemplar improvement candidates.
...
If `--create` is the first argument, run this bootstrap sequence and then **stop** — do not
proceed to Step 1. This mode is idempotent: safe to re-run on an already-initialised project.
